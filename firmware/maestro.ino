/**
 * MAESTRO (ESP32-BRIDGE) - Optimizado
 * CSV: slaveID,timestamp_us,red,ir,ax,ay,az,gx,gy,gz,instantaneousFs,battery_mv
 * battery_mv se imprime en cada línea (misma Fs que el resto de los datos),
 * pero su valor solo cambia cada ~15 s porque el esclavo lo cachea.
 */
...
void OnDataRecv(const esp_now_recv_info_t * info, const uint8_t *incomingData, int len) {
  ...
  if (len == sizeof(struct_data_packet)) {
    ...
    // Desempaquetado para la interfaz de Python
...
void setup() {
  ...
  esp_wifi_set_ps(WIFI_PS_NONE); // <- Fundamental para evitar cuelgues del Wi-Fi
